  ...
  allReqsRef
        ...
        return usersRef.once("value", (snapshot) => {
          if (!snapshot.hasChild(userID)) {
            console.log(
              "User with ID " +
                userID +
                " was not found. Could not delete its SR reference."
            );
            return;
          }
    ...
    .then(() => {
      res
        .status(200)
        .send(
          "Successfully deleted SR from Open, Closed, All SR and reference from its author."
        );
      return;
    })
    .catch((error) => {
...
function removeFromRef(dbRef, uuid, refDescription) {
  return dbRef.once("value", (snapshot) => {
    ...
    return dbRef
      ...
      .catch((error) => {
        console.log(
          "Remove from " + refDescription + " failed: " + error.message
        );
      });
  });
}
  ...
  query.once("value", (snapshot) => {
    snapshot.forEach(function (childSnapshot) {
      ...
      if (chval === uuid) {
        ...
        console.log(
          "Deleted reference of SR with uuid " + uuid + " from its author"
        );
        return true;
      }
    });
