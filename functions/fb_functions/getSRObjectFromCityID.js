...
exports.handler = (req, res, allReqsRef, usersRef) => {
	...
	if (!secret) {
		...
		return;
	} else if (secret !== constants.GET_SR_BY_ID_SECRET) {
	...
	if (!cityIDsToQuery) {
		res.status(400).send('You must pass an array of \'requestIds\' as a parameter.');
		return;
	...
	.then(srArray => {
		return bluebird.Promise.map(srArray, sr => {
			return usersRef.child(sr.submittedBy).once('value')
			.then(snapshot => {
