...
exports.handler = (
  ...
  openReqsRef.once("value", (snapshot) => {
    if (snapshot.hasChild(uuid)) {
      // Request found
      helpersModule.moveDbChild(
        openReqsRef.child(uuid),
        closedReqsRef.child(uuid)
      );
      ...
      allReqsRef.child(uuid).once("value", (requestSnapshot) => {
        ...
        if (!fullSR) {
          res
            .status(500)
            .send("Could not find full SR in database with uuid " + uuid);
          return;
        }
        ...
        if (!userId) {
          res
            .status(500)
            .send("Could not find 'submittedBy' in SR with uuid " + uuid);
          return;
        }

        helpersModule.sendMessageToUser(
          userId,
          fullSR.serviceName,
          usersRef,
          admin
      ...
      res
        ...
        .send(
          "SR with uuid " +
            uuid +
            " could not be found in the open requests node."
        );
    }
  });
};
